export class ScrollHorizontalManager {
  /**
  ...
  constructor(params = {}) {
    ...
    //Manage multi Link navigation
    if (this.params.slideLink === true) {
      this.linkContainer = document.querySelectorAll(
        `[data-${this.params.sliderName}nav]`
      );
      this.linkCollection = [];
      for (const key in this.linkContainer) {
        if (Object.hasOwnProperty.call(this.linkContainer, key)) {
          const container = this.linkContainer[key];

          this.linkCollection[key] = container.querySelectorAll(
    ...
    console.error(this.params.sliderName);
    ...
    this.drawSliderPage();
    this.resizeSlider();
    if (this.params.keyBoardControl === true) {
      this.keyBoardControl();
    }
    if (this.params.slideXWithMouseWheel === true) {
      this.slider();
    }
    if (this.params.touchAndDrag === true) {
      this.manageDrag();
    }
  }
  ...
  drawSliderPage() {
    ...
    for (const key in this.slideCollection) {
      if (Object.hasOwnProperty.call(this.slideCollection, key)) {
        ...
        if (this.params.sizeSlide === true) {
          slideItem.style.minWidth = `${this.params.slideWidth}${this.params.unity}`;
        }
        ...
        if (
          ...
          this.params.sizeSlide === true
        ) {
          slideItem.style.minWidth = `100${this.params.unity}`;
        }
      }
    }

    //Associate anchor link with slide ID, valid for multi link Navigation
    if (this.params.slideLink === true) {
      for (const linkCollection of this.linkCollection) {
        linkCollection[0].classList.add(`active`);
  ...
  /*Nav with link NAV , valid for multi Link Navigation*/
  slideLink() {
    for (const linkCollection of this.linkCollection) {
      linkCollection[0].classList.add(`active`);
  ...
  jumpToSlide(idx) {
    ...
    this.scrollContainer.style.transform = `translate3d(-${value}${this.params.unity},0,0)`;
    setTimeout(() => {
      this.enabledTransition();
    }, 100);
  }
  ...
  goToNext() {
    console.log(this.params.currentSlide);
  ...
  resizeSlider() {
    ...
    if (
      ...
      this.slideWrapper.style.minHeight = `${sliderHeight}px`;
